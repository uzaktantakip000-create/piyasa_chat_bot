...
def test_bulk_settings_update_normalizes_profile(authenticated_client):
    ...
    response = client.put('/settings/bulk', json=payload)
...
def test_patch_setting_updates_single_key(authenticated_client):
    client = authenticated_client
...
def test_bulk_update_requires_payload(authenticated_client):
    response = authenticated_client.put('/settings/bulk', json={'updates': {}})
    assert response.status_code == 400
    assert 'No updates' in response.text



def test_patch_news_feed_urls_validates_and_deduplicates(authenticated_client):
    client = authenticated_client
    response = client.patch(
...
def test_patch_news_feed_urls_rejects_invalid_urls(authenticated_client):
...
def test_metrics_handles_wrapped_setting_values(authenticated_client):
